...
class GreedyFrontier():
    ...
    def add(self, node):
        ...
        heapq.heappush(self.frontier, (h, next(self.counter), node))
...
class AstarFrontier(GreedyFrontier):
    ...
    def add(self, node):
        h = abs(node.state[0] - self.goal[0]) + abs(node.state[1] - self.goal[1])
        g = self.calculate_g(node)
        f = h + g
        heapq.heappush(self.frontier, (f, next(self.counter), node))
...
class Maze():
    ...
    def solve(self):
        ...
        elif (self.algo).lower() == 'bfs':
            frontier = QueueFrontier()
        elif self.algo.lower() == "greedy":
            frontier = GreedyFrontier(self.goal)
        elif self.algo.lower() == "astar":
            frontier = AstarFrontier(self.goal)

        else:
            raise ValueError("Unknown algorithm: use 'dfs', 'bfs', 'greedy' or 'astar'")
